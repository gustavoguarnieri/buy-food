...
public class EstablishmentPaymentWayService {
    ...
    @Autowired
    public EstablishmentPaymentWayService(ModelMapper modelMapper, PaymentWayRepository paymentWayRepository, StatusValidation statusValidation) {
    ...
    public List<PaymentWayResponseDTO> getPaymentWayList(Integer status) {
        if (status == null) {
            ...
            return paymentWayRepository.findAllByStatus(statusValidation.getStatusIdentification(status)).stream()
                    .map(this::convertToDto)
                    .collect(Collectors.toList());
        }
    }
