...
public class EstablishmentBusinessHoursService {
    ...
    @Autowired
    public EstablishmentBusinessHoursService(ModelMapper modelMapper, BusinessHoursRepository businessHoursRepository, EstablishmentService establishmentService, UserService userService, StatusValidation statusValidation) {
    ...
    public List<EstablishmentBusinessHoursResponseDTO> getMyBusinessHoursList(Integer status) {
        if (status == null) {
            return businessHoursRepository.findAllByAuditCreatedBy(userService.getUserId().orElse("-1"))
                    .stream()
                    .map(this::convertToDto)
                    .collect(Collectors.toList());
        } else {
    ...
    public EstablishmentBusinessHoursResponseDTO createBusinessHours(
            Long establishmentId, EstablishmentBusinessHoursRequestDTO establishmentBusinessHoursRequestDto) {
