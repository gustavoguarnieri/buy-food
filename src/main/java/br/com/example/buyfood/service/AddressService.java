...
public class AddressService {
    ...
    @Autowired
    public AddressService(ModelMapper modelMapper, DeliveryAddressRepository deliveryAddressRepository, UserService userService, StatusValidation statusValidation) {
    ...
    public List<DeliveryAddressResponseDTO> getAddressListByCreatedBy(Integer status) {
        return deliveryAddressRepository
                .findAllByAuditCreatedByAndStatus(getUserId(), statusValidation.getStatusIdentification(status))
                .stream()
                .map(this::convertToDto)
                .collect(Collectors.toList());
    }
    ...
    public DeliveryAddressEntity getUserAddressById(Long addressId) {
        return deliveryAddressRepository.findById(addressId)
                .orElseThrow(() -> new NotFoundException(ErrorMessages.ADDRESS_NOT_FOUND));
    }
