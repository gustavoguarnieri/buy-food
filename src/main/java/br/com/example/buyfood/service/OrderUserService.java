...
public class OrderUserService {
  ...
  @Autowired private PreparationStatusRepository preparationStatusRepository;
  ...
  public List<OrderResponseDTO> getOrderList(Integer status) {
    if (status == null) {
      ...
      switch (status) {
        case 1:
          return orderUserRepository.findAllByStatus(RegisterStatus.ENABLED.getValue()).stream()
              .map(this::convertToDto)
              .collect(Collectors.toList());
        case 0:
          {
            return orderUserRepository.findAllByStatus(RegisterStatus.DISABLED.getValue()).stream()
                .map(this::convertToDto)
                .collect(Collectors.toList());
          }
        default:
