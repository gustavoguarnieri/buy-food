...
public class ProductImageService {
  ...
  public List<ImageResponseDTO> getProductImageList(
    ...
    if (status == null) {
      ...
      switch (status) {
        case 1:
          return getProductImageListByEstablishmentIdProductIdAndStatus(
              establishment, productId, RegisterStatus.ENABLED);
        case 0:
          {
            return getProductImageListByEstablishmentIdProductIdAndStatus(
                establishment, productId, RegisterStatus.DISABLED);
          }
        default:
  ...
  public ImageResponseDTO createProductImage(
      Long establishmentId, Long productId, MultipartFile file) {
  ...
  private ImageResponseDTO saveImage(
      MultipartFile file, EstablishmentEntity establishment, ProductEntity product) {
    var downloadPath = getDownloadProductPath(establishment, product);

    var uploadFileResponse =
        fileStorageService.saveFile(
            file, FileStorageFolder.PRODUCTS, product.getId(), downloadPath);
  ...
  public List<ImageResponseDTO> createProductImageList(
      Long establishmentId, Long productId, MultipartFile[] files) {
    var establishment = establishmentService.getEstablishmentById(establishmentId);
  ...
  public ResponseEntity<Resource> getDownloadProductImage(
    ...
    return fileStorageService.downloadFile(
        FileStorageFolder.PRODUCTS, productId, fileName, request);
  }
  ...
  private List<ImageResponseDTO> getProductImageListByEstablishmentIdProductIdAndStatus(
      EstablishmentEntity establishment, Long productId, RegisterStatus enabled) {
    return productImageRepository
        .findAllByProductIdAndStatus(productId, enabled.getValue())
        .stream()
        .filter(i -> i.getProduct().getEstablishment().getId().equals(establishment.getId()))
        .map(this::convertToDto)
        .collect(Collectors.toList());
