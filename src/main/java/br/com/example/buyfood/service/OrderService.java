...
public class OrderService {
    ...
    public OrderResponseDTO getOrder(Long orderId) {
        var establishmentOrder = orderRepository.findById(orderId)
                .orElseThrow(() -> new NotFoundException(ErrorMessages.USER_NOT_FOUND));
        return convertToDto(establishmentOrder);
    }

    public List<OrderResponseDTO> getOrderList(Integer status) {
        return orderRepository.findAllByStatus(statusValidation.getStatusIdentification(status))
                .stream()
    ...
    public List<OrderResponseDTO> getOrderListByCreatedBy(Integer status) {
        return orderRepository.findAllByAuditCreatedByAndStatus(
