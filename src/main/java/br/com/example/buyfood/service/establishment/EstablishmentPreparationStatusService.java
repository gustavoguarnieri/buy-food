...
public class EstablishmentPreparationStatusService {
    ...
    @Autowired
    public EstablishmentPreparationStatusService(ModelMapper modelMapper, PreparationStatusRepository preparationStatusRepository, StatusValidation statusValidation) {
    ...
    public List<PreparationStatusResponseDTO> getPreparationStatusList(Integer status) {
        if (status == null) {
            ...
            return preparationStatusRepository.findAllByStatus(statusValidation.getStatusIdentification(status)).stream()
                    .map(this::convertToDto)
                    .collect(Collectors.toList());
        }
    }
