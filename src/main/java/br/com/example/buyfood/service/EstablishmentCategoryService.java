...
public class EstablishmentCategoryService {
    ...
    public EstablishmentCategoryResponseDTO createEstablishmentCategory(EstablishmentCategoryRequestDTO establishmentCategoryRequestDto) {
    ...
    private List<EstablishmentCategoryResponseDTO> getEstablishmentCategoryListByStatus(RegisterStatus enabled) {
        return establishmentCategoryRepository.findAllByStatus(enabled.getValue()).stream()
                .map(this::convertToDto)
                .collect(Collectors.toList());
    }

    private EstablishmentCategoryResponseDTO convertToDto(EstablishmentCategoryEntity establishmentCategoryEntity) {
        return modelMapper.map(establishmentCategoryEntity, EstablishmentCategoryResponseDTO.class);
    }

    private EstablishmentCategoryEntity convertToEntity(EstablishmentCategoryRequestDTO establishmentCategoryRequestDto) {
